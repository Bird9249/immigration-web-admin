...
import getLostPassportByIdApi from "./apis/get-lost-passport-by-id.api";
import updateLostPassportApi from "./apis/update-lost-passport.api";
import {
  UpdateLostPassportSchema,
  UpdateLostPassportSchemaType,
} from "./schemas/update-lost-passport.schema";
...
export default () => {
  ...
  if (!checkPermission(Permission.Write, PermissionGroup.LostPassport, auth))
    navigator(-1);

  const [accommodationRequest] = createResource(
    param.id,
    getLostPassportByIdApi
  );
  ...
  const [form, { Form, FieldArray, Field }] =
    createForm<UpdateLostPassportSchemaType>({
      validate: valiForm(UpdateLostPassportSchema),
      initialValues: {
        translates: [
          {
            id: 0,
  ...
  createEffect(
    on(
      ...
      (input) => {
        if (input) {
          const data: UpdateLostPassportSchemaType = {
            translates: [
              {
  ...
  const handleSubmit: SubmitHandler<UpdateLostPassportSchemaType> = async (
    values
  ) => {
    const res = await updateLostPassportApi(param.id, values);
    actionMessage.showMessage({ level: "success", message: res.data.message });
    navigator("/lost-passport");
  };

  return (
    <div class="relative">
      <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">
        ແກ້ໄຂຂໍ້ມູນໜັງສືຜ່ານແດນເສຍ
      </h2>
