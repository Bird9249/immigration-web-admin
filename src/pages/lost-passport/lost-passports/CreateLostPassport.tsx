...
import createLostPassportApi from "./apis/create-lost-passport.api";
import {
  CreateLostPassportSchema,
  CreateLostPassportSchemaType,
} from "./schemas/create-lost-passport.schemas";
...
export default () => {
  ...
  const [form, { Form, FieldArray, Field }] =
    createForm<CreateLostPassportSchemaType>({
      validate: valiForm(CreateLostPassportSchema),
      initialValues: {
        translates: [
          {
            title: "",
  ...
  const handleSubmit: SubmitHandler<CreateLostPassportSchemaType> = async (
    values
  ) => {
    const res = await createLostPassportApi(values);
