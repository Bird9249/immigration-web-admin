...
import { NewCategoriessResponse, NewCategoriessTableState } from "./api/news-categories.interface";
...
export default () => {
    ...
    const [NewsCategoriess, { refetch }] = createResource(state, getNewsCategoriesApi);
    ...
    const actionMenus = (id: number) => {
        ...
        if (checkPermission(Permission.Remove, PermissionGroup.NewsCategoriess, auth)) {
            menus.push([]);
    ...
    return (
        ...
        >
            {[
                ...
                {
                    header: "ເວລາອັບເດດ",
                    body: ({ updated_at }: NewCategoriessResponse) => (
