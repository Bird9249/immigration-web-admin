...
export default () => {
    ...
    return (
        <div class="relative">
            <Show when={newsCategoriess()} fallback={<div class="h-[470px]"></div>}>
                {(item) => (
                    <Tabs
                                ...
                                <div><br />
                                    ...
                                    <div class="px-4 mb-4 grid gap-4 sm:mb-5 sm:grid-cols-2 sm:gap-6 md:gap-12">
                                        <dl>
                                            ...
                                            <dd class="text-gray-500 dark:text-gray-400 font-light mb-4 sm:mb-5">
                                                <Show when={newsCategoriess()?.data.created_at} fallback={"ບໍ່ມີຂໍ້ມູນເວລາສ້າງ"}>
                                                    {(hotel_created_at) =>
                                                        format(
                                                            hotel_created_at(),
                                                            "dd/MM/yyyy HH:mm:ss"
                                            ...
                                            <dd class="text-gray-500 dark:text-gray-400 font-light mb-4 sm:mb-5">
                                                <Show when={newsCategoriess()?.data.updated_at} fallback={"ບໍ່ມີຂໍ້ມູນເວລາອັບເດດ"}>
                                                    {(hotel_updated_at) =>
                                                        format(
                                                            hotel_updated_at(),
                                                            "dd/MM/yyyy HH:mm:ss"
            ...
            <div class="p-4 flex items-center">
                <Show
                    when={checkPermission(Permission.Write, PermissionGroup.NewsCategoriess, auth)}
                >
                    <Button
                        class="mr-3"
                        color="primary"
                        prefixIcon={<EditIcon />}
                        onClick={() => {
                            navigator(`/newsCategoriess/edit/${param.id}`);
                        }}
                    >
                        ແກ້ໄຂ
                    </Button>
                ...
                <Show
                    when={checkPermission(Permission.Remove, PermissionGroup.NewsCategoriess, auth)}
                >
                    <Button
                        color="danger"
