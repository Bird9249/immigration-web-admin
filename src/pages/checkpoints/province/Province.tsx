...
export default () => {
    ...
    if (!checkPermission(Permission.Read, PermissionGroup.Province, auth))
        navigate(-1);
    ...
    const actionMenus = (id: number) => {
        ...
        if (checkPermission(Permission.Remove, PermissionGroup.Province, auth)) {
            menus.push([]);
        ...
        <Table
            header={
                <div class="flex flex-col items-start justify-between border-b dark:border-gray-600 p-4 sm:flex-row sm:items-center">
                    ...
                    <Show
                        when={checkPermission(Permission.Write, PermissionGroup.Province, auth)}
                    >
                        <Button
