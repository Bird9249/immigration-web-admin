...
import registerExitActionApi from "../apis/register-exit-action.api";
import registerExitNumberApi from "../apis/register-exit-number.api";
...
export default () => {
  ...
  const [data, { refetch }] = createResource(registerExitNumberApi);

  const submit: SubmitHandler<NumberSchemaType> = async (values) => {
    const res = await registerExitActionApi(values, action);
