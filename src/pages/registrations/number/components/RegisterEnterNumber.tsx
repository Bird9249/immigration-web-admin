...
import registerEnterActionApi from "../apis/register-enter-action.api";
import registerEnterNumberApi from "../apis/register-enter-number.api";
...
export default () => {
  ...
  const [data, { refetch }] = createResource(registerEnterNumberApi);

  const submit: SubmitHandler<NumberSchemaType> = async (values) => {
    const res = await registerEnterActionApi(values, action);
        ...
        <dl class="grid max-w-screen-md gap-8 mx-auto text-gray-900 sm:grid-cols-3 dark:text-white">
          <div class="flex flex-col items-center justify-center">
            <dt class="mb-2 text-2xl md:text-3xl font-extrabold">
              {data() ? data()?.data.per_day.toLocaleString() : "..."}
            </dt>
          ...
          <div class="flex flex-col items-center justify-center">
            <dt class="mb-2 text-2xl md:text-3xl font-extrabold">
              {data() ? data()?.data.per_mouth.toLocaleString() : "..."}
            </dt>
            <dd class="font-light text-gray-500 dark:text-gray-400">
              ເຂົ້າ 30 ມື້ກ່ອນ
            </dd>
