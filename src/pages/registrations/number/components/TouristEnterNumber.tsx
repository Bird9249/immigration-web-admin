...
import touristEnterActionApi from "../apis/tourist-enter-action.api";
import touristEnterNumberApi from "../apis/tourist-enter-number.api";
...
export default () => {
  ...
  const [data, { refetch }] = createResource(touristEnterNumberApi);

  const submit: SubmitHandler<NumberSchemaType> = async (values) => {
    const res = await touristEnterActionApi(values, action);
